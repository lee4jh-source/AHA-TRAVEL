  ...
  <main className="pt-24 px-6 max-w-md mx-auto pb-32">
    <section className="mb-10">
      ...
      <h1 className="text-[2rem] font-semibold leading-tight text-[#041627] mb-4">
        해외여행 필수 문장 <span className="text-[#a93100]">어디서든 당당하게!</span>
      </h1>
      <div className="h-1.5 w-16 bg-[#a93100] mb-8"></div>
    </section>
...
const SpeakingModal = ({ queue, onClose }: { queue: any[], onClose: () => void }) => {
  ...
  const toggleRecording = () => {
    ...
    if (isRecording) {
      ...
      if (recognizedText) {
        const calculatedScore = calculateScore(queue[idx].en, recognizedText);
...
const LearningTab = ({ category, setRecentSentences }: { category: string, setRecentSentences: (sentences: any[]) => void }) => {
  ...
  const fetchMore = async () => {
    ...
    const promise = generateMoreSentences(data.title, existingEn).then(newSentences => {
  ...
  }, [currentPage, sentences.length, data.title]);
    ...
    return (
      <main className="max-w-md mx-auto px-6 pt-24 pb-32">
        <section className="mb-10">
          ...
          <h1 className="text-[2rem] font-semibold leading-tight text-[#041627] mb-4">
            방금 학습한 문장 <span className="text-[#a93100]">완벽하게 익히기</span>
          </h1>
          <div className="h-1.5 w-16 bg-[#a93100] mb-8"></div>
...
const GuideTab = () => (
  <main className="max-w-md mx-auto px-6 pt-24 pb-32">
    <section className="mb-10">
      ...
      <h1 className="text-[2rem] font-semibold leading-tight text-[#041627] mb-4">
        해외여행 가이드 <span className="text-[#a93100]">완벽 준비하기</span>
      </h1>
      <div className="h-1.5 w-16 bg-[#a93100] mb-8"></div>
